...
DEFAULT_ADV_SHARES: float = 1e6
MAX_PARTICIPATION_RATE: float = 0.10
SQRT_IMPACT_COEFFICIENT: float = 0.1
...
class MarketImpactModel:
    ...
    def estimate_cost(
        ...
        impact = (
            self.permanent_impact_coef * daily_vol * np.power(participation_rate, self.impact_exponent)
        )
...
class QuantConfig:
    ...
    max_position_pct_adv: float = 0.05
...
def compute_transaction_cost_vectorized(
    ...
    if HAS_NUMBA:
        ...
        for i, asset in enumerate(asset_names):
            ...
            impact_cost = (
                impact_model.temporary_impact_coef * daily_vol * np.power(participation, impact_model.impact_exponent) * trade_size
            )
            total_cost += spread_cost + impact_cost
...
def main() -> None:
    ...
    try:
        portfolio_results.to_parquet("backtest_results.parquet")
        actual_weights_df.to_parquet("actual_weights.parquet")
